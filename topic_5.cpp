...
void set_perspective_matrix(double fovy_degrees, double aspect, double near, double far)
{
  ...
  matrix[0] = 2.0 / (2.0 - -2.0);
  matrix[5] = 2.0 / (2.0 - -2.0);
  matrix[10] = -2.0 / (10.0 - 1.0);
  matrix[12] = -(2.0 + -2.0) / (2.0 - -2.0);
  matrix[13] = -(2.0 + -2.0) / (2.0 - -2.0);
  matrix[14] = -(10.0 + 1.0) / (10.0 - 1.0);
  */

  GLfloat f = 1/tan(fovy_degrees * (M_PI / 180.0) / 2);
  ...
  //gluPerspective(fovy_degrees, aspect, near, far);
}

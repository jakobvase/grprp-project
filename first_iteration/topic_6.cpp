...
vector<vertex> clip_polygon(vector<vertex> input_vertex_list, ClipDirection direction, float position)
{
  ...
  if (input_vertex_list.size() == 0)
    return output_vertex_list;
  
  vertex last = input_vertex_list.at(input_vertex_list.size() - 1);
  for (int i = 0; i < input_vertex_list.size(); ++i) {
    vertex current = input_vertex_list.at(i);
    if (is_inside(current, direction, position)) {
      if (!is_inside(last, direction, position)) {
        output_vertex_list.push_back(intersect(last, current, direction, position));
      }
      output_vertex_list.push_back(current);
    }
    else if (is_inside(last, direction, position)) {
      output_vertex_list.push_back(intersect(last, current, direction, position));
    }
    last = current;
    
    
    // If any vertex is inside, return whole polygon unchanged.
    //if (is_inside(input_vertex_list.at(i), direction, position)) {
    //  output_vertex_list = input_vertex_list;
    //  return output_vertex_list;
    //}
  }

  // If still here, return an empty polygon.
  return output_vertex_list;
}

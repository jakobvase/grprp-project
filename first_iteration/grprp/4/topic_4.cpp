...
void shade_vertex(vertex v, vertex n)
{
  ...
  result_r += Kd_r * nonnegative_dot(n, v_to_l);
  result_g += Kd_g * nonnegative_dot(n, v_to_l);
  result_b += Kd_b * nonnegative_dot(n, v_to_l);

  // ADD SPECULAR COMPONENT.  (Try using nonnegative_dot() and pow().)
  result_r += Ks_r * pow(nonnegative_dot(v_to_eye, l_reflect), Kexp);
  result_g += Ks_g * pow(nonnegative_dot(v_to_eye, l_reflect), Kexp);
  result_b += Ks_b * pow(nonnegative_dot(v_to_eye, l_reflect), Kexp);
  /**/
